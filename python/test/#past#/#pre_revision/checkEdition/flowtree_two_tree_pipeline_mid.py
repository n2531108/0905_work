...
# -*- coding: utf-8 -*-
"""
flowtree_two_tree_pipeline_mid.py
...
前提:
  ot_estimators_python（中間版C++）が build 済みであり、少なくとも次が見えていること:
    - load_vocabulary(...)
...
使い方例:
  python3 flowtree_packed_1d_pipeline.py \
    ...
    --so_dir   "/mnt/c/Users/成見/0905_work/native/build_mid" \
    --qid 0 \
    --doc_id 0 \
    --seedA 110 \
    --seedB 111
...
def import_ot_estimators_module(so_dir: str, module_name: str = "ot_estimators_python"):
    """
    中間版C++モジュールを読み込む。

    注意:
      sys.path は insert(0) にして、既存 build/ 側の .so を誤って読まないようにする。
    """
    if so_dir in sys.path:
...
def build_two_ots(so_dir: str, vocab: np.ndarray, seedA: int, seedB: int, module_name: str = "ot_estimators_python"):
    mod = import_ot_estimators_module(so_dir, module_name=module_name)
...
# ============================================================
# 11.5) dot 出力（中間C++で取得した packed subtree をPython側で可視化）
# ============================================================

def _dot_escape(s: str) -> str:
    # Graphviz の label 内では \n を改行として使いたいので、バックスラッシュはエスケープしない。
    # ダブルクォートだけをエスケープする。
    return s.replace('"', '\\"')


def write_packed_subtree_dot(
    res: SingleTreeResult,
    path: str,
    title: str,
) -> None:
    """
    中間C++から取得した packed subtree を Graphviz dot として保存する。

    最新版C++の debug_dump_tree*_dot と同じ考え方で、
      - packed node id
      - vocab id
      - Algorithm A の x座標
      - delta
    ...
    注意:
      Graphviz上の横位置は packed_x 順ではなく、自動レイアウト。
      packed_x はラベル中の x=... として確認する。
    """
    os.makedirs(os.path.dirname(path), exist_ok=True)

    packed = res.packed
    leaf_x = res.leaf_x_by_node
    children = children_from_edges(packed.n_nodes, packed.edges)

    with open(path, "w", encoding="utf-8") as f:
        f.write("digraph packed_tree {\n")
        f.write("  rankdir=TB;\n")
        f.write("  node [shape=circle, fontsize=10];\n")
        f.write("  labelloc=\"t\";\n")
        f.write(f"  label=\"{_dot_escape(title)}\";\n")

        for u in range(packed.n_nodes):
            is_leaf = bool(packed.is_leaf[u])
            if is_leaf:
                vid = int(packed.unleaf[u]) if 0 <= u < len(packed.unleaf) else -1
                delta = float(packed.delta[u]) if 0 <= u < len(packed.delta) else 0.0
                x = leaf_x.get(u, -1)
                if delta > 0:
                    color = "lightblue"
                elif delta < 0:
                ...
                label = f"u={u}\\nvid={vid}\\nx={x}\\ndelta={delta:.9g}"
                f.write(
                    f"  n{u} [shape=box, style=filled, fillcolor=\"{color}\", "
                    f"label=\"{_dot_escape(label)}\"];\n"
                )
            else:
                f.write(f"  n{u} [label=\"u={u}\"];\n")

        f.write(f"  n{packed.root} [peripheries=2];\n")
        for u in range(packed.n_nodes):
            for v in children[u]:
                f.write(f"  n{u} -> n{v};\n")
    ...
    print(f"[dot] wrote {path}")


def print_dot_hints(dot_paths: List[str]) -> None:
    if not dot_paths:
        return
    print("[dot hint]")
    for path in dot_paths:
        svg_path = os.path.splitext(path)[0] + ".svg"
        print(f"  dot -Tsvg {path} -o {svg_path}")
    print()
...
def main() -> None:
    ...
    ap.add_argument("--so_dir", required=True, help="directory containing ot_estimators_python*.so")
    ap.add_argument("--module_name", default="ot_estimators_python", help="pybind module name for the intermediate C++ build")
    ...
    ap.add_argument("--dump_dot", action="store_true", help="write packed subtree dot files for treeA/treeB")
    ap.add_argument("--dot_dir", default="/mnt/c/Users/成見/0905_work/python/test", help="directory to save dot files")
    args = ap.parse_args()

    print("[module]")
    print(f"  so_dir      = {args.so_dir}")
    print(f"  module_name = {args.module_name}")
    print()
    ...
    # 候補生成用。build_packed_subtree() を直接呼ぶので、この2つは候補生成専用にする。
    ...
    # コスト評価用。中間C++では build_packed_subtree() 単独呼び出し後に
    # 同じインスタンスで flowtree_distance_pair() を呼ぶと marked/num_queries が干渉しうるため、
    # baseline/residual 評価は別インスタンスで行う。
    otA_cost, otB_cost, _ = build_two_ots(
        so_dir=args.so_dir,
        vocab=vocab,
        seedA=args.seedA,
        seedB=args.seedB,
    ...
    if args.dump_dot:
        ...
        pathA = os.path.join(
            args.dot_dir,
            f"mid_treeA_q{args.qid}_d{args.doc_id}_seed{args.seedA}.dot",
        )
        pathB = os.path.join(
            args.dot_dir,
            f"mid_treeB_q{args.qid}_d{args.doc_id}_seed{args.seedB}.dot",
        )
        write_packed_subtree_dot(
            resA,
            pathA,
            title=f"mid treeA qid={args.qid} doc_id={args.doc_id} seed={args.seedA}",
        )
        write_packed_subtree_dot(
            resB,
            pathB,
            title=f"mid treeB qid={args.qid} doc_id={args.doc_id} seed={args.seedB}",
        )
        dot_paths.extend([pathA, pathB])
        print_dot_hints(dot_paths)
    ...
    flowtree_cost_pair(otA_cost, rem_plus, rem_minus)
    flowtree_cost_pair(otB_cost, rem_plus, rem_minus)
    flowtree_cost_pair(otA_cost, query_mass, doc_mass)
    flowtree_cost_pair(otB_cost, query_mass, doc_mass)

    # 値そのものは1回だけ取得
    residualA = flowtree_cost_pair(otA_cost, rem_plus, rem_minus)
    residualB = flowtree_cost_pair(otB_cost, rem_plus, rem_minus)
    baselineA = flowtree_cost_pair(otA_cost, query_mass, doc_mass)
    baselineB = flowtree_cost_pair(otB_cost, query_mass, doc_mass)

    """"
    # warm-up
    for _ in range(warmup):
        _ = flowtree_cost_pair(otA_cost, rem_plus, rem_minus)
        _ = flowtree_cost_pair(otB_cost, rem_plus, rem_minus)
        _ = flowtree_cost_pair(otA_cost, query_mass, doc_mass)
        _ = flowtree_cost_pair(otB_cost, query_mass, doc_mass)
    """
    ...
    if not seed_api_ok:
        print("[note] 中間C++側の load_vocabulary は seed 引数を受け取っていません。")
