...
目的:
  ...
  - [MOD] プレマッチ後の残差を C++ 側 FlowTree に渡し、
          prematch cost + residual FlowTree cost を計算する
...
def extract_doc_block(text: str, doc_id: int) -> Tuple[DocHeader, str]:
    """
    指定 doc_id のブロックを抜き出す。
    ブロック範囲は
      '=== doc_id=... ===' から次の '=== doc_id=... ===' 手前まで。
    """
...
def parse_nn_table_from_block(block: str) -> Dict[int, NNRow]:
    """
    ブロック内の NN表行 (u_id u_x v_id v_x d1) をすべて拾って u_id をキーにする。
    """
...
def build_compare_rows(
    ...
    nnB: Dict[int, NNRow],
    vocab: np.ndarray,
    priority: str = "A",  # fixed by user request
    ...
    # priority by A.d1 asc, then A.u_x, then u_id
    if priority != "A":
...
def greedy_prematch(
    ...
    """
    rem_plus = measure_to_dict(q_measure)   # query side mass (a)
    rem_minus = measure_to_dict(d_measure)  # doc side mass (b)
...
# ============================================================
# [MOD] residual / cost helpers
# ============================================================

def residual_dict_to_measure(rem: Dict[int, float], eps: float = 1e-12) -> List[Tuple[int, float]]:
    """
    [MOD] rem_plus / rem_minus の dict を
          FlowTree に渡せる list[(vid, mass)] に変換
    """
    out: List[Tuple[int, float]] = []
    for vid, mass in sorted(rem.items()):
        m = float(mass)
        if m > eps:
            out.append((int(vid), m))
    return out
...
def prematch_cost(match_edges: List[MatchEdge]) -> float:
    ...
    for e in match_edges:
        s += float(e.flow) * float(e.l2_pick)
    return s

def residual_flowtree_cost(
    so_dir: str,
    vocab: np.ndarray,
    residual_doc_measure: List[Tuple[int, float]],
    residual_query_measure: List[Tuple[int, float]],
    seed: int,
) -> float:
    """
    [MOD DEBUG] C++ 側の flowtree_distance_pair() を使って
                residual query / residual doc の FlowTree コストを計算
    """
    print("[PY DEBUG] enter residual_flowtree_cost")
    print(f"[PY DEBUG] seed={seed}")
    print(f"[PY DEBUG] len(residual_query_measure)={len(residual_query_measure)}")
    print(f"[PY DEBUG] len(residual_doc_measure)={len(residual_doc_measure)}")
    print(f"[PY DEBUG] sum_q={sum(w for _, w in residual_query_measure):.12f}")
    print(f"[PY DEBUG] sum_d={sum(w for _, w in residual_doc_measure):.12f}")
    print(f"[PY DEBUG] first q[:10]={residual_query_measure[:10]}")
    print(f"[PY DEBUG] first d[:10]={residual_doc_measure[:10]}")

    if so_dir not in sys.path:
        sys.path.append(so_dir)
    print("[PY DEBUG] sys.path append done")

    import ot_estimators  # type: ignore
    print("[PY DEBUG] import ot_estimators done")

    ot = ot_estimators.OTEstimators()
    print("[PY DEBUG] created OTEstimators")

    ot.load_vocabulary(vocab.astype(np.float32), seed)
    print("[PY DEBUG] load_vocabulary done")

    ot.prepare_pair_mode()
    print("[PY DEBUG] prepare_pair_mode done")

    cost = float(ot.flowtree_distance_pair(residual_query_measure, residual_doc_measure))
    print("[PY DEBUG] flowtree_distance_pair done")
...
def print_costs(cost_pre: float, cost_res: float, cost_total: float):
    """
    [MOD] コスト表示
    """
    print("[cost]")
    print(f"  prematch_cost          = {cost_pre:.6f}")
    print(f"  residual_flowtree_cost = {cost_res:.6f}")
    print(f"  total_cost             = {cost_total:.6f}")
    print()
...
def main():
    ...
    ap.add_argument("--so_dir", required=True, help="dir containing ot_estimators*.so")  # [MOD]
    ...
    # ============================================================
    # [MOD] residual FlowTree cost と total cost を計算
    # ============================================================

    rem_plus_measure = residual_dict_to_measure(rem_plus)
    rem_minus_measure = residual_dict_to_measure(rem_minus)

    cost_pre = prematch_cost(match_edges)
    cost_res = residual_flowtree_cost(
        so_dir=args.so_dir,
        vocab=vocab,
        residual_doc_measure=rem_minus_measure,
        residual_query_measure=rem_plus_measure,
        seed=hdrA.seed if hdrA.seed is not None else 0,
    )
    cost_total = cost_pre + cost_res

    print_costs(cost_pre, cost_res, cost_total)
